...
#sudo killall node &> /dev/null
#sudo killall node &> /dev/null
kill $(ps aux | grep node | grep -v "testmode" | grep -v "  $1" | awk '{print $2}') &> /dev/null
kill $(ps aux | grep node | grep -v "testmode" | grep -v "  $1" | awk '{print $2}') &> /dev/null
kill $(ps aux | grep node | grep -v "testmode" | grep -v "  $1" | awk '{print $2}') &> /dev/null
...
if [ "$#" == 0 ] || [ "$#" -gt 1 ] ; then
    ...
    while true; do killall Xvfb; killall Xvfb; Xvfb -ac -screen scrn 1280x2000x24 :9.0 & disown; echo "<pre>$(date +%Y-%m-%d_%H-%M-%S) Restart Xvfb..." >> static/log.html; sleep $((60 * 2)); done &>> /dev/null & disown
