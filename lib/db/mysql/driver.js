...
const hash              = rootrequire('lib', 'hash');
const log               = rootrequire('lib', 'log');
const abstract          = require(path.resolve(__dirname, 'db_abstract'));
const searchforerrors   = rootrequire('lib', 'db', 'searchforerrors');
...
module.exports = function (config, general) {
    ...
    if (!config.idColumn) {
        throw "config.idColumn is not specified";
    }

    if (!config.table) {
        throw "config.table is not specified";
    }

    var tables = {
        pool: pool,
        cache: extend(config.table, config.idColumn, {
            /**
             * fetch next url to process
             */
            fetch: function () {
                return this.query("select * from :table: WHERE updateRequest is not null ORDER BY updateRequest DESC LIMIT 1")
                    .then((rows) => {
                        return rows[0] || null;
                    });
            },
