    ...
    return new Promise((resolve, reject) => {
        if (args.length > 1) {
            if (typeof select !== 'string') {
                return reject({message:'input error', error:'second argument of find method should be string'});
            }
        }
        else {
            select = '*';
        ...
        var query = 'SELECT ' + select + ' FROM :table: WHERE `' + key + '` = :' + key;

        return this.query(query, id).then(function (rows) {
